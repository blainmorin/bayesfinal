...
Summary statistics for the capacity indicators by the type of agency are shown in Table 1. Agency types are assigned based on their primary spending patterns. For example, the Environmental Protection Agency (EPA) allocates a majority of its budget to "Natural Resources and Environment" and is categorized accordingly. As we see in the table, the type with the highest number of different agencies is Education. Across time, health agencies employed to highest number of people on average. National defense agencies had the highest median salary across years. General science and basic research employed the highest percentage of people with a Master's Degree or higher level of education. Crime, Law Enforcement, and Incarceration the highest (as a percentage of agency employment) average requisitioning rate from other agencies. General Government agencies lost the highest percentage of agency employment on average over the years. Average lengths of service and gini coefficient were similar across agency types.  
...
Figure 2 displays the trends of the capacity indicators over time for all agencies. Environmental agencies are highlighted in green and the EPA is highlighted in red. We see that within agency employment for environmental agencies has stayed relatively constant. We see that median salaries have a slightly upward trend (although these are not inflation adjusted). The average length of service within environmental agencies is increasing over time. 

We see that environmental agencies lose more employees (either to other agencies or other sectors) at a faster rate than they gain employees from other agencies. For the EPA, we see that the gini coefficient decreases over the past 40 years. Because the average length of service and median salary has been increasing over time with the number of employees relatively constant, this declining gini coefficient may signal a sort of hiring freeze. In other words, the higher observed median salary and lower gini coefficient could be caused by a declining rate of new entry level hires.   

Lastly, the correlations between the capacity indicators are checked in Figure 3. We see that a number of the indicators are moderately strong correlations. The strongest positive correlation is between median salary and the percentage of employees with a Master's degree or higher, which makes intuitive sense. The strongest negative correlation is between median salary and the gini coefficient. 
...
where $\mathbf{y}_{i,t}$ is a vector of indicators. $\mathbf{F}_{i,t}$ is a state transition matrix, which in our scenario is assumed to be equal to the identity matrix. Moreover, the latent process evolves such that the latent state at time $t$ is a function of the state at time $t-1$:
$$  \mathbf{Z}_{i,t} = \mathbf{G}_{i,t}\mathbf{Z_{i, t-1}} + \mathbf{u}_{i,t} $$
For our capacity process, we start by assuming that capacity of an agency follows a random walk, or in other words that $\mathbf{G}_{i,t} = \mathbf{I}, \forall i,t$ [@jackman]. We estimate this model in rstan using a marginal likelihood Kalman filter.
